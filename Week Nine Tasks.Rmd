...
pop.decline <- function(df, ntimes){
  ...
  geom_point() +
  geom_smooth(method='glm',formula=y~x, method.args = list(family = "poisson"), color
      = "orange") +
  ...
  resid.plot1 <- plot(best_model$residuals, main = "Residual Plot for Best Model",
                      ylab = "Residuals")
  ...
  #sim_reg <- apply(y.rep.sort, 2, lm())#problem!!!
  ...
  #return stuff
  structure(list(aic_summary, end.date))
  return(list(plot1, resid.plot1))
}
```
